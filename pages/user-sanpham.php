...
<div class="sanpham">
 ...
 <div class="details">
    <div class="nametree">
      ...
      <h3 id="giá"><?php echo number_format($product['Price'], 0, ',', '.') . " VND"; ?></h3>
      
      <?php $tonKho = (int)$product['StockQuantity']; ?>
      <?php if ($tonKho > 0): ?>
          <p style="font-size: 14px; color: #555; margin-top: 10px;">
              Sản phẩm có sẵn: <strong style="color: #28a745;"><?= $tonKho ?></strong>
          </p>
      <?php else: ?>
          <p style="font-size: 15px; color: red; font-weight: bold; margin-top: 10px;">
              Đã hết hàng
          </p>
      <?php endif; ?>
    </div>
    ...
    <div class="order">
      <?php if ($tonKho > 0): ?>
          <div class="wrapper">
            ...
            <span class="num" id="display-num">01</span>
          ...
          <script>
              ...
              btnPlus.addEventListener('click', () => {
                  if (currentQty < maxStock) {
                      ...
                      // Báo lỗi nếu cố tình bấm vượt quá kho
                      alert("Rất tiếc, trong kho hiện chỉ còn " + maxStock + " sản phẩm!");
                  }
              });

              function updateDisplay() {
                  // Thêm số 0 ở trước nếu nhỏ hơn 10 cho đẹp (giống UI cũ của bạn)
                  displayNum.innerText = currentQty < 10 ? "0" + currentQty : currentQty;
                  inputQty.value = currentQty;
              }
          </script>
      <?php else: ?>
          <button type="button" class="btn btn-secondary btn-lg" disabled style="cursor: not-allowed; background-color: #ccc; border: none;">Tạm thời hết hàng</button>
      <?php endif; ?>
    </div>
  </div>
</div>
